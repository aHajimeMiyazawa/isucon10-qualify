...
func initialize(c echo.Context) error {
	...
	paths := []string{
		...
		filepath.Join(sqlDir, "mod/index_nazotte.sql"),
		filepath.Join(sqlDir, "mod/idx_low_priced.sql"),
	}
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ?
						AND ST_Contains(ST_PolygonFromText(%s), ST_GeomFromText(CONCAT('POINT(', CAST(latitude AS CHAR), ' ', CAST(longitude AS CHAR), ')'))) ORDER BY popularity_desc ASC, id ASC`,
